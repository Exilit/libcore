		...
		if ((base = spool_path(MAGMA_SPOOL_BASE)) && !spool_check(base) && !spool_check(path)) {
			...
			if ((template = st_aprint("%.*s%s_%lu_%lu_XXXXXX", st_length_int(path), st_char_get(path), prefix, thread_get_thread_id(), _rand_get_uint64()))
				...
				if (((now = time(NULL)) - spool_creation_failure) > 3600) {
					log_critical("Temporary file creation failed. {mkstemp = %i / file = %.*s}", fd, st_length_int(template), st_char_get(template));
...
int_t spool_check_file(const char *file, const struct stat *info, int type) {
	...
	if (type == FTW_F) {
		if (unlink(file)) {
			log_error("An error occurred while trying to unlink a temporary file inside the spool. {%s / %s}", strerror_r(errno, _tbufptr, _tbuflen), file);
			mutex_lock(&spool_error_lock);
			spool_errors++;
			mutex_unlock(&spool_error_lock);
		}
...
bool_t spool_start(void) {
	...
	for (int_t i = 0; i < 3 && result == true; i++) {
		...
		else if (i == MAGMA_SPOOL_SCAN) path = spool_path(MAGMA_SPOOL_SCAN);
